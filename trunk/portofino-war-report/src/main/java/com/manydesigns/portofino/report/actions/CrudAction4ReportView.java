...
public class CrudAction4ReportView extends CrudAction4Report {
    ...
    public Resolution exportFromList(final DocType docType) {
    ...
    public Resolution export(final DocType docType) {
        String reportId = pk[0];
        return export(docType, reportId);
    }

    public Resolution export(final DocType docType, final String reportId) {
        ...
        QueryArgsPojo queryParamPojo = new QueryArgsPojo();
        queryParamPojo.setDocType(docType);
        return reportView.export(context.getRequest(), t_reportPojo, queryParamPojo, getReturnUrl());
    }

    @Button(list = "crud-search", key = "commons.exportHtml", order = 10, group = "crudExport", icon = "icon-share",
        type = Button.TYPE_WARNING)
